<?php
/**
 * Plugin Name:       Sticky WhatsApp Button
 * Description:       Adds a floating WhatsApp button to the bottom-right corner of your WordPress site.
 * Version:           1.1.0
 * Author:            Your Name
 * Text Domain:       sticky-whatsapp
 */

defined( 'ABSPATH' ) || exit;

/**
 * Enqueue plugin styles and scripts
 */
function sticky_whatsapp_enqueue_assets() {
    $plugin_url = plugin_dir_url( __FILE__ );

    // Get phone number from settings or use default
    $phone = get_option( 'sticky_whatsapp_phone', '526421005010' );

    wp_enqueue_style(
        'sticky-whatsapp-style',
        $plugin_url . 'assets/style.css',
        array(),
        '1.0.0'
    );

    wp_enqueue_script(
        'sticky-whatsapp-script',
        $plugin_url . 'assets/script.js',
        array(),
        '1.0.0',
        true
    );

    // Pass phone number to JS
    wp_localize_script( 'sticky-whatsapp-script', 'StickyWhatsAppData', array(
        'phone' => $phone,
    ));
}
add_action( 'wp_enqueue_scripts', 'sticky_whatsapp_enqueue_assets' );

/**
 * Render WhatsApp button in the footer
 */
function sticky_whatsapp_render_button() {
    ?>
    <a href="#" class="sticky-whatsapp" id="whatsapp-btn" target="_blank" rel="noopener noreferrer" aria-label="Chat on WhatsApp">
        <svg width="32" height="32" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M18.5986 3.17871C16.5527 1.12793 13.8281 0 10.9326 0C4.95605 0 0.0927734 4.86328 0.0927734 10.8398C0.0927734 12.749 0.59082 14.6143 1.53809 16.2598L0 21.875L5.74707 20.3662C7.3291 21.2305 9.11133 21.6846 10.9277 21.6846H10.9326C16.9043 21.6846 21.875 16.8213 21.875 10.8447C21.875 7.94922 20.6445 5.22949 18.5986 3.17871ZM10.9326 19.8584C9.31152 19.8584 7.72461 19.4238 6.34277 18.6035L6.01562 18.4082L2.60742 19.3018L3.51562 15.9766L3.30078 15.6348C2.39746 14.1992 1.92383 12.5439 1.92383 10.8398C1.92383 5.87402 5.9668 1.83105 10.9375 1.83105C13.3447 1.83105 15.6055 2.76855 17.3047 4.47266C19.0039 6.17676 20.0488 8.4375 20.0439 10.8447C20.0439 15.8154 15.8984 19.8584 10.9326 19.8584ZM15.874 13.1104C15.6055 12.9736 14.2725 12.3193 14.0234 12.2314C13.7744 12.1387 13.5938 12.0947 13.4131 12.3682C13.2324 12.6416 12.7148 13.2471 12.5537 13.4326C12.3975 13.6133 12.2363 13.6377 11.9678 13.501C10.376 12.7051 9.33105 12.0801 8.28125 10.2783C8.00293 9.7998 8.55957 9.83398 9.07715 8.79883C9.16504 8.61816 9.12109 8.46191 9.05273 8.3252C8.98437 8.18848 8.44238 6.85547 8.21777 6.31348C7.99805 5.78613 7.77344 5.85938 7.60742 5.84961C7.45117 5.83984 7.27051 5.83984 7.08984 5.83984C6.90918 5.83984 6.61621 5.9082 6.36719 6.17676C6.11816 6.4502 5.41992 7.10449 5.41992 8.4375C5.41992 9.77051 6.3916 11.0596 6.52344 11.2402C6.66016 11.4209 8.43262 14.1553 11.1523 15.332C12.8711 16.0742 13.5449 16.1377 14.4043 16.0107C14.9268 15.9326 16.0059 15.3564 16.2305 14.7217C16.4551 14.0869 16.4551 13.5449 16.3867 13.4326C16.3232 13.3105 16.1426 13.2422 15.874 13.1104Z" fill="white"/>
</svg>
    </a>
    <?php
}
add_action( 'wp_footer', 'sticky_whatsapp_render_button' );

/**
 * Add plugin settings menu in admin panel
 */
function sticky_whatsapp_add_admin_menu() {
    add_options_page(
        'Sticky WhatsApp Settings',     // Page title
        'Sticky WhatsApp',              // Menu title
        'manage_options',               // Capability
        'sticky-whatsapp',              // Slug
        'sticky_whatsapp_settings_page' // Callback
    );
}
add_action( 'admin_menu', 'sticky_whatsapp_add_admin_menu' );

/**
 * Register plugin settings
 */
function sticky_whatsapp_register_settings() {
    register_setting( 'sticky_whatsapp_settings_group', 'sticky_whatsapp_phone', [
        'type' => 'string',
        'sanitize_callback' => 'sanitize_text_field',
        'default' => '526421005010',
    ]);
}
add_action( 'admin_init', 'sticky_whatsapp_register_settings' );

/**
 * Render the settings page HTML
 */
function sticky_whatsapp_settings_page() {
    ?>
    <div class="wrap">
        <h1>Sticky WhatsApp Settings</h1>
        <form method="post" action="options.php">
            <?php
            settings_fields( 'sticky_whatsapp_settings_group' );
            do_settings_sections( 'sticky_whatsapp_settings_group' );
            ?>
            <table class="form-table">
                <tr valign="top">
                    <th scope="row">WhatsApp Phone Number</th>
                    <td>
                        <input
                            type="text"
                            name="sticky_whatsapp_phone"
                            value="<?php echo esc_attr( get_option( 'sticky_whatsapp_phone', '526421005010' ) ); ?>"
                            class="regular-text"
                        />
                        <p class="description">Enter the full phone number including country code (e.g. 521234567890).</p>
                    </td>
                </tr>
            </table>
            <?php submit_button(); ?>
        </form>
    </div>
    <?php
}
